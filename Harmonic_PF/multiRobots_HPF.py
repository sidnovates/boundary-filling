"""
Swarm Robotics: Multi-Robot Target-Line Filling with Harmonic PF
This simulation preserves the original 100x100 environment and U-shaped obstacle,
but replaces the global APF target attraction with a harmonic potential field.
The goal remains the same: robots occupy the target line and stop.
"""
...
ATT_FORCE_MAX = 15  # Upper bound on attractive force magnitude for more natural motion
...
ATT_TOTAL_MAX = 12  # Hard cap on total point-attraction magnitude (base + near boost)
...
class Robot:
    ...
    def __init__(self, x, y, env_width, env_height, radius=ROBOT_RADIUS):
        ...
        self.harmonic_force_gain = HARMONIC_FORCE_GAIN
    ...
    def compute_occupied_point_repulsion(self, target_points, point_occupied, point_occupants):
        field = np.zeros(2)
        for idx, point in enumerate(target_points):
            if not point_occupied[idx]:
                continue
            if point_occupants[idx] is self:
                continue
            ...
            dist = math.hypot(dx, dy)
            if dist <= 0.001 or dist > PARKED_POINT_REPULSE_RANGE:
                continue

            ux = dx / dist
            uy = dy / dist
            falloff = max(0.0, 1.0 - dist / PARKED_POINT_REPULSE_RANGE)
            magnitude = PARKED_POINT_REPULSE_SCALE * self.att_total_max * falloff
            field += np.array([ux, uy]) * magnitude

        return field

    ## Building Wall - Utility Function
    ...
    ##how much clearence is there from the wall
...
#### CCollision Prevention System #########
    ##For collision Checking - "what-if" collision checker
...
## Computes all forces with harmonic global guidance and local avoidance
    def compute_force_components(self, env, guidance_point):
        ...
        f_robot = self.compute_robot_repulsion(env.robots, guidance_point)
        f_wall = self.compute_wall_repulsion(env.inner_walls, env.target_line)
        ...
        f_parked_points = self.compute_occupied_point_repulsion(
            env.target_points,
            env.target_point_occupied,
            env.target_point_occupants,
        )

        f_total = f_random + f_robot + f_wall + f_harmonic + f_parked_points
    ...
    def update(self, env):
        ...
        force_components = self.compute_force_components(env, guidance_point)
...
def main():
    ...
    ax.set_aspect('equal')
    ax.set_title("Multi-Robot Harmonic PF - Line Seeking")
    ...
    force_lines = {
        ...
        "harmonic": ax.plot([], [], color='green', linewidth=1.8, zorder=5, label='F_harmonic')[0],
        "parked_points": ax.plot([], [], color='red', linewidth=1.6, zorder=5, label='F_parked')[0],
    ...
    stats_text = ax.text(
        0.01,
        0.99,
        "Click a robot to inspect harmonic guidance",
    ...
    def update_frame(_):
        ...
        if selected['index'] is None:
            ...
            for line in force_lines.values():
                line.set_data([], [])
            stats_text.set_text("Click a robot to inspect harmonic guidance")
        else:
            ...
            _set_force_line(force_lines["harmonic"], sx, sy, comp["harmonic"])
            _set_force_line(force_lines["parked_points"], sx, sy, comp["parked_points"])
            ...
            stats_text.set_text(
                ...
                f"line_dist={comp['line_distance']:.2f}, harmonic_U={comp['harmonic_potential']:.3f}, grad|U|={comp['harmonic_grad_norm']:.4f}\n"
                f"|F_random|={np.linalg.norm(comp['random']):.3f}, |F_robot|={np.linalg.norm(comp['robot_repulsion']):.3f}\n"
                f"|F_wall|={np.linalg.norm(comp['wall_repulsion']):.3f}, |F_harmonic|={np.linalg.norm(comp['harmonic']):.3f}, |F_parked|={np.linalg.norm(comp['parked_points']):.3f}\n"
